...
// a single WebView can't wrap native chrome around its own web content,
// so the browser bar (back / forward / reload / address / go / close) is injected into EVERY document from an embedded script (Scripts\BrowserChrome.js),
...
public partial class MainWindow : AOTrinoWindow
{
    ...
    // StartUrl is the whole hook: AOTrinoWindow navigates here once the controller is up,
    // so a browser that opens on a page someone asked for is one override, not a navigation of its own.
