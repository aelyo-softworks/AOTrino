...
// a reduced local file explorer. stays NavigationMode.Local (the default): browsing goes through the fs host
// object, and the "AOTrino on GitHub" link in the page — a real <a href> — is cancelled by the Local nav-lock
// and opened in the system browser instead. that contrast with the WebBrowser sample is the whole point.
...
public partial class MainWindow : AOTrinoWindow
{
    ...
    // the preview pane loads local files (file://) in an <iframe>; allow a file:// page to reach other local
    // files. safe here: the window stays Local (it never loads remote content) and this is the developer's
