...
// how the WebView2 bridge reshapes a .NET host object seen from JS: every member is asynchronous,
// methods return a Promise and property reads are themselves Promises. Awaited<> collapses the
// double wrapping for a C# method that already returns Task<T>.
...
function unavailable<T extends object>(name: string): T {
    // throw on first use rather than on creation, so a module-scope `const api = host<Api>()` stays
    // safe in a plain browser and callers can still branch on isHosted()
...
// T describes the object's JS-visible surface; names cross the bridge case-insensitively, so
// camelCase in TypeScript matches the PascalCase members in C#.
