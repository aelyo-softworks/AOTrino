...
const useStyles = makeStyles({
    ...
    //
    // this goes on a div INSIDE FluentProvider, never on FluentProvider itself. the provider copies its own
    // className onto every portal mount node it creates (applyStylesToPortals, on by default, and needed:
    // it's what themes popups). put `height: 100vh` there and each portal becomes a full-window node - which,
    // carrying the theme's background too, paints an opaque rectangle at z-index 1000000 over the whole app
...
export interface AOTrinoProviderProps {
    ...
    // localStorage needs a real origin, which is why AOTrinoWindow.VirtualHostName matters here: a file://
    // page has an opaque origin and the choice would not survive a restart. see docs/THEMING.md.
...
// FluentProvider, wired to the Windows app theme and sized like a window rather than a page, plus the theme
// choice that <TitleBar>'s picker drives.
// this is the opinionated layer: it exists to make the common desktop case a one-liner. Everything it does is
// a choice you can undo - pass `theme` to pin one, `themes` to offer your own, `storageKey: null` to forget.
export function AOTrinoProvider({
    ...
    const value = useMemo<AOTrinoThemeContextValue>(() => {
        const pinned = !!theme;
        // "system" resolves by matching the OS light/dark against the offered themes, so a custom list still
        // gets system-following as long as it has a dark one and a light one
